    ...
    if(dataObject){
      ...
      if(route == "login") {
        if(dataObject["userName"]) {
          if (url.charAt(url.length - 1) == "?") {
            url += "userName=" + dataObject["userName"];
          } else {
            url += "&userName=" + dataObject["userName"];
          }
        ...
        if(dataObject["password"]) {
          if (url.charAt(url.length - 1) == "?") {
            url += "password=" + dataObject["password"];
          } else {
            url += "&password=" + dataObject["password"];
          }
...
(function(root) {
  ...
  NCMB.Query.prototype = {
   ...
   /** NCMB.Queryの制限にマッチしないようにする制限をクエリに追加する。
     * 
    ...
    /**
     * ほかのNCMB.Queryの制限にマッチするオブジェクトの値にキーの値をマッチしない制限をクエリに追加する。
     * 
     * @param {String} key マッチする値を含まれるキー名
     * @param {String} queryKey 再マッチするためクエリから取得したオブジェクトのキー名
     * @param {NCMB.Query} query 実行するクエリ
